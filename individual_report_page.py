...
# 脑区中文名的唯一数据源（与 Agent、RAG 共用）
from brain_region_names import NODE_NAME_CN, to_cn_region
...
class IndividualReportPage(QWidget):
    def __init__(self, main_window=None):
        ...
        self.label_file = get_resource_path("label_order_jian.node")
        self.coord_file = get_resource_path("Node_AAL116.node")
    ...
    def _resolve_inputs(self):
        """优先用诊断页处理好的数组；缺失时回退到 Agent 的 CaseStore。"""
        bold = self.case_data.get("bold_data")
        fc = self.case_data.get("fc_data")
        if bold is not None and fc is not None:
            return fc, bold
        from agent.skills.predict import CaseStore, prepare_inputs, load_matrix_file

        store = CaseStore()
        case_id = self.case_data.get("case_id") or store.latest()[0]
        case = store.get(case_id) if case_id else {}
        if case.get("fc_file") and case.get("bold_file") \
                and os.path.exists(case["fc_file"]) and os.path.exists(case["bold_file"]):
            return np.load(case["fc_file"]), np.load(case["bold_file"])
        if case.get("fc_path") and case.get("bold_path"):
            return prepare_inputs(
                load_matrix_file(case["fc_path"], case.get("fc_var", "")),
                load_matrix_file(case["bold_path"], case.get("bold_var", "")),
            )
        return None, None

    def _resolve_model_device(self, num_nodes: int):
        """诊断页模型优先，否则用 Agent 侧的模型单例"""
        page = getattr(self.main_window, "intelligent_prediction", None)
        model = getattr(page, "model", None)
        device = getattr(page, "device", None)
        if model is not None and device is not None:
            return model, device
        from agent.skills.predict import get_model, get_device
        return get_model(num_nodes=num_nodes, num_time_points=130), get_device()
    ...
    def run_individual_analysis(self):
        try:
            ...
            # 1. 取与训练方式对齐的输入（诊断页数组优先，缺失时回退 CaseStore）
            fc, bold = self._resolve_inputs()
            pred = self.case_data.get("prediction") or 0
            pred_prob = self.case_data.get("pred_prob", 0)

            if bold is None or fc is None:
                raise ValueError("未接收到 BOLD 或 FC 数据，也找不到已保存的病例数据")

            # 2. 模型与设备（页面优先，否则用 Agent 的模型单例）
            model, device = self._resolve_model_device(int(bold.shape[0]))
            if model is None:
